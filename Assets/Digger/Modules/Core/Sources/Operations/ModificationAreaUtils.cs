...
namespace Digger.Modules.Core.Sources.Operations
{
    public static class ModificationAreaUtils
    {
        public static ModificationArea GetSphericalAreaToModify(DiggerSystem digger, float3 position, float radius)
        {
            ...
            var voxelRadius = (int)((radius + Math.Max(digger.CutMargin.x, digger.CutMargin.z)) /
                                    Math.Min(digger.HeightmapScale.x, digger.HeightmapScale.z)) + 1;
            var voxMargin = new Vector3i(voxelRadius, (int)((radius + digger.CutMargin.y) / digger.HeightmapScale.y) + 1, voxelRadius);
